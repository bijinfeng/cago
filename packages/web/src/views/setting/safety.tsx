  ...
  return (
    <div className="max-w-[668px] mx-auto">
      ...
      <div className="mt-4 space-y-4">
        {sessions.map(item => (
          <div key={item.id} className="flex items-center p-4 rounded-md bg-secondary text-muted-foreground gap-4">
            ...
            <Button variant="outline" className="text-xs leading-6 px-2 py-0 h-auto" disabled={item.isCurrent}>删除</Button>
          </div>
        ))}
      </div>
        ...
        <TableBody>
          {auditlogs.map(item => (
            <TableRow key={item.id}>
              ...
              <TableCell className="text-muted-foreground">{dayjs(item.createAt).format("YYYY-MM-DD HH:mm:ss")}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
