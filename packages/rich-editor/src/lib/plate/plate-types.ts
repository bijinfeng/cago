...
export function getMyEditor(editor: MyEditor) {
  return getTEditor<MyValue, MyEditor>(editor)
}
...
export function createMyPlateEditor(options: CreatePlateEditorOptions<MyValue, MyEditor> = {}) {
  return createPlateEditor<MyValue, MyEditor>(options)
}
export function createMyPluginFactory<P = PluginOptions>(defaultPlugin: PlatePlugin<NoInfer<P>, MyValue, MyEditor>) {
  return createPluginFactory(defaultPlugin)
}
export function createMyPlugins(plugins: PlatePlugin[], options?: {
  components?: Record<string, PlatePluginComponent>
  overrideByKey?: OverrideByKey
}) {
  return createPlugins<MyValue, MyEditor>(plugins, options)
}
